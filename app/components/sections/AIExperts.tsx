...
function CategoryIcon({ name, color }: { name: string; color: string }) {
  ...
  const paths: Record<string, React.ReactNode> = {
    Ryan: <><circle cx="12" cy="12" r="3.2" /><path d="M12 4v2M12 18v2M4 12h2M18 12h2M6.5 6.5l1.4 1.4M16.1 16.1l1.4 1.4M17.5 6.5l-1.4 1.4M7.9 16.1l-1.4 1.4" /></>,
  ...
  return (
    <span className="shrink-0 rounded-lg flex items-center justify-center" style={{ width: 28, height: 28, background: `${color}1a`, border: `1px solid ${color}44` }}>
...
function Avatar({ src, name, color, size = 52 }: { src: string; name: string; color: string; size?: number }) {
  return (
    <span
      ...
      style={{ width: size, height: size, background: "radial-gradient(circle at 50% 35%, #1a3b74, #0a1730)", boxShadow: `0 0 0 1.5px ${color}66, 0 0 16px ${color}55` }}
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={src} alt={name} width={size} height={size} className="object-cover w-full h-full" loading="lazy" onError={(e) => { (e.currentTarget as HTMLImageElement).style.opacity = "0"; }} />
    </span>
  );
}
...
/* ── Featured (centre) card — animates when the expert changes ── */
function FeaturedCard({ expert }: { expert: Expert }) {
  return (
    <div
      ...
      style={{
        ...
        border: `1.5px solid ${expert.color}80`,
        boxShadow: `0 0 60px ${expert.color}45, inset 0 0 46px rgba(30,70,180,0.2)`,
      }}
    >
      <span className="inline-flex items-center gap-1.5 rounded-full mb-4" style={{ padding: "5px 14px", background: `${expert.color}26`, border: `1px solid ${expert.color}73`, fontFamily: "var(--font-body)", fontSize: 11, fontWeight: 700, color: "#e6eeff" }}>
        <span aria-hidden style={{ color: expert.color }}>★</span> Featured Expert
      </span>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="relative mb-4" style={{ width: 150, height: 150 }}>
            <div aria-hidden className="absolute inset-0 rounded-full" style={{ border: `1.5px solid ${expert.color}73`, boxShadow: `0 0 34px ${expert.color}80, inset 0 0 30px rgba(40,90,200,0.35)` }} />
            <div aria-hidden className="absolute rounded-full" style={{ inset: 10, border: `1px solid ${expert.color}33` }} />
            <div className="absolute overflow-hidden rounded-full" style={{ inset: 14, background: "radial-gradient(circle at 50% 32%, #1c47a0, #0a1730)" }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={expert.image} alt={expert.name} className="object-cover w-full h-full" onError={(e) => { (e.currentTarget as HTMLImageElement).style.opacity = "0"; }} />
            </div>
          </div>

          <p style={{ fontFamily: "var(--font-heading)", fontSize: 30, fontWeight: 800, color: "#ffffff", lineHeight: 1.05 }}>{expert.name}</p>
          <p style={{ fontFamily: "var(--font-body)", fontSize: 14, fontWeight: 700, color: expert.color, marginTop: 3 }}>{expert.specialty}</p>
          <p style={{ fontFamily: "var(--font-body)", fontSize: 12.5, fontWeight: 400, color: "rgba(199,210,220,0.7)", lineHeight: 1.45, marginTop: 10 }}>{expert.blurb}</p>

          <div className="flex flex-wrap justify-center gap-1.5 mt-4 pt-4 w-full" style={{ borderTop: "1px solid rgba(120,150,255,0.15)" }}>
            {expert.tags.map((t) => (
              <span key={t} style={{ padding: "5px 10px", borderRadius: 8, background: "rgba(120,150,255,0.1)", border: "1px solid rgba(120,150,255,0.25)", fontFamily: "var(--font-body)", fontSize: 10, color: "rgba(210,220,235,0.85)" }}>{t}</span>
            ))}
          </div>
        </motion.div>
      </AnimatePresence>
...
function PlatformBase() {
  return (
    <div aria-hidden className="absolute left-1/2 -translate-x-1/2 pointer-events-none" style={{ bottom: 28, width: 360, height: 150 }}>
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className="absolute left-1/2 -translate-x-1/2 rounded-[50%]" style={{ bottom: 10 + i * 12, width: 340 - i * 74, height: 74 - i * 15, border: "1px solid rgba(90,140,255,0.28)", boxShadow: "0 0 18px rgba(40,110,255,0.15)" }} />
      ))}
      <div className="absolute left-1/2 -translate-x-1/2 rounded-full animate-pulse-glow" style={{ bottom: 22, width: 120, height: 40, background: "radial-gradient(ellipse, rgba(90,170,255,0.85), rgba(40,90,220,0.25) 55%, transparent 75%)", filter: "blur(6px)" }} />
      <div className="absolute left-1/2 -translate-x-1/2" style={{ bottom: 30, width: 60, height: 130, background: "linear-gradient(to top, rgba(90,170,255,0.4), transparent)", filter: "blur(10px)", borderRadius: "50%" }} />
    </div>
  );
}
  ...
  return (
    <section id="features" className="relative overflow-hidden" style={{ background: "transparent" }}>
      <style>{`
        ...
        /* pause the orbit while hovering so cards are easy to click */
        .ai-orbit-stage:hover .ai-orbit-rotator,
        .ai-orbit-stage:hover .ai-orbit-counter { animation-play-state: paused; }
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 md:px-[80px] pt-16 md:pt-24 pb-20 md:pb-28">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-8">
          ...
          <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left max-w-[520px]">
            <motion.span initial={{ opacity: 0, y: -10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="inline-flex items-center rounded-full glass neon-border" style={{ padding: "6px 16px", fontFamily: "var(--font-body)", fontSize: 12, fontWeight: 600, color: "#9ec8f5" }}>
              {AI_EXPERTS_BADGE}
            </motion.span>

            <motion.h2 initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(2.1rem, 4.5vw, 3.1rem)", fontWeight: 800, letterSpacing: "-1px", lineHeight: 1.1, color: "#e8eef6", marginTop: 20 }}>
              {AI_EXPERTS_HEADING_PRE}
              <span className="gradient-text text-glow-blue">{AI_EXPERTS_HEADING_HIGHLIGHT}</span>
            </motion.h2>

            <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} style={{ fontFamily: "var(--font-body)", fontSize: 16, fontWeight: 400, lineHeight: "27px", color: "rgba(199,210,220,0.65)", marginTop: 18 }}>
              {AI_EXPERTS_SUBHEADING}
            </motion.p>

            <motion.a href="#paths" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }} className="inline-flex items-center gap-2 rounded-xl mt-8 transition-transform duration-200 hover:scale-[1.03] active:scale-[0.98]" style={{ padding: "13px 26px", background: "linear-gradient(135deg, #3b6fff 0%, #00c2ff 100%)", boxShadow: "0 0 26px rgba(59,111,255,0.4)", fontFamily: "var(--font-body)", fontSize: 15, fontWeight: 700, color: "#fff" }}>
              {AI_EXPERTS_CTA}
          ...
          <div className="ai-orbit-stage relative hidden md:block flex-shrink-0" style={{ width: 660, height: 660, maxWidth: "100%" }}>
            ...
            <div className="ai-orbit-rotator absolute inset-0" style={{ animation: `aiOrbitSpin ${SPIN}s linear infinite` }}>
              {slots.map((name, i) => {
                ...
                return (
                  <div key={i} className="absolute top-1/2 left-1/2" style={{ transform: `translate(-50%, -50%) rotate(${angle}deg) translateX(${RADIUS}px) rotate(${-angle}deg)` }}>
                    <div className="ai-orbit-counter" style={{ animation: `aiOrbitSpinRev ${SPIN}s linear infinite` }}>
                      <OrbitCard expert={byName(name)} onSelect={() => selectSlot(i)} />
                    </div>
                  </div>
                );
              })}
            ...
            {/* featured pinned in the centre */}
            <div className="absolute top-1/2 left-1/2 z-10" style={{ transform: "translate(-50%, -55%)" }}>
              <FeaturedCard expert={featured} />
            </div>

            {/* "Learn Anytime" chip */}
            <div className="absolute z-10 flex items-center gap-2.5 rounded-xl" style={{ left: 0, bottom: 40, padding: "10px 14px", background: "linear-gradient(150deg, rgba(20,26,64,0.9), rgba(10,14,38,0.94))", border: "1px solid rgba(120,150,255,0.28)", boxShadow: "0 10px 30px rgba(0,0,0,0.4)" }}>
